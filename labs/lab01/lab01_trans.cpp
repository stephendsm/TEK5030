...
// We are using Eigen and OpenCV to transform image by applying transformation matrices on homogeneous form.
...
int main()
{
  ...
  Eigen::Matrix2d R;
  R << Eigen::Rotation2Dd(deg2rad(angle_deg)).toRotationMatrix();
  ...
  u_transformed << (E * u.homogeneous()).hnormalized();
  ...
  // Compose the transformation to find the resulting similarity transform.
